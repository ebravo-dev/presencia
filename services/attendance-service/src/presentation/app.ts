...
import { issueBindingToken } from '../infrastructure/binding-token.js';
import type { AttendanceEnv } from '../infrastructure/config.js';
import { captureAttendanceSchema, coordinatorBindingSchema, coordinatorUnbindSchema, deviceBindingSchema, rosterSnapshotSchema } from './schemas.js';
...
export async function buildAttendanceApp(options: {
  ...
  app.setErrorHandler((error, request, reply) => {
    ...
    if (error instanceof AttendanceDomainError) {
      ...
      const forbidden = error.code === 'PROFESSOR_GROUP_FORBIDDEN';
      const notFound = error.code === 'ATTENDANCE_GROUP_NOT_FOUND';
      return reply.code(conflict.includes(error.code) ? 409 : forbidden ? 403 : notFound ? 404 : 400).send({ error: error.code, message: error.message });
    }
