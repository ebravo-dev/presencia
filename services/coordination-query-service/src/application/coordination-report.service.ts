...
export class CoordinationReportService {
  constructor(
    ...
    private readonly toleranceMinutes = 10,
  ) {}
  ...
  async weekly(teacherId: string, weekStart: string) {
    ...
    const rows = source.groups
      ...
      .flatMap((group) => this.weeklyRow(group, dates));
    ...
    return {
      ...
      meta: { generatedAt: this.now().toISOString(), timezone: 'America/Mexico_City' },
    };
  }
  ...
  async range(teacherId: string, startDate: string, endDate: string) {
    ...
    const rows = source.groups.filter((group) => matchesCycle(group, cycles)).map((group) => {
      ...
      const attended = scheduled.filter((block) => {
        ...
        return this.blockStatus(block, record) === 'TAKEN' || this.blockStatus(block, record) === 'LATE';
      }).length;
  ...
  private weeklyRow(group: GroupSource, dates: Array<{ day: Day; date: string }>) {
    const schedule = normalizeSchedule(group.schedule);
    ...
    const cells = Object.fromEntries(dates.map(({ day, date }) => {
      ...
      const hourSlots = blocks.map((block, index) => ({
        index, startTime: block.startTime, endTime: block.endTime, status: this.blockStatus(block, record),
      }));
  ...
  private blockStatus(block: HourBlock, record?: AttendanceRecord): CellStatus {
    ...
    const onTime = record.professorEntryAt.getTime() <= block.start.getTime() + this.toleranceMinutes * 60_000;
...
function emptyCell(dateValue: string, status: CellStatus) {
  return { date: dateValue, status, professorEntryAt: null, professorExitAt: null, scheduledHours: 0, attendedHours: 0, coverageRate: null, hourSlots: [] as Array<{ index: number; startTime: string; endTime: string; status: CellStatus }>, portalSyncStatus: null, portalSyncError: null };
}
