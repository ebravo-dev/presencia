...
| Captura local y subida posterior a UAT | La presencia del profesor entra únicamente por el canal de beacon/entrada/salida y usa hora del servidor; la captura de alumnos no puede inyectar timestamps de profesor. Attendance usa transacción serializable, idempotencia y outbox. El titular queda `PENDING`; una clase compartida queda `SKIPPED` porque no puede usar credenciales ajenas. UAT Integration cifra credenciales y reintenta. | Implementado; falta caos contra UAT real. |
