...
describe('DashboardPage', () => {
  it('muestra los conteos y cobertura recibidos del backend', async () => {
    ...
    server.use(http.get('/api/coordinacion/infraestructura/resumen', () => HttpResponse.json({
      data: {
        counts: { beacons: 3, studentDeviceBindings: 7, studentBleAttendances: 14, activeSubstitutions: 1 },
        recentBindings: [],
        recentSubstitutions: [],
        recentBeacons: [],
      },
    ...
    expect(screen.getByText('Beacons de salón').previousElementSibling).toHaveTextContent('3');
  });
});
