  ...
  return <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#111111] px-4 py-8 sm:px-6">
    ...
    <section className="relative z-10 w-full max-w-[460px] rounded-xl border border-white/50 bg-white px-7 py-7 shadow-2xl shadow-black/40 sm:px-10 sm:py-8">
      ...
      <form className="mt-7 space-y-4" onSubmit={submit}>
        <div><label className="label" htmlFor="email">Correo institucional</label><div className="relative"><Mail className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17}/><input className="field field-leading-icon border-red-200 bg-red-50/35" id="email" type="email" autoComplete="username" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="coordinacion@uat.edu.mx"/></div></div>
