...
type BindingTab = 'students' | 'professors';

export function DeviceBindingsPage() {
  const queryClient = useQueryClient();
  const user = useAuthStore((state) => state.user);
  const [search, setSearch] = useState('');
  const [activeTab, setActiveTab] = useState<BindingTab>('students');
  const [message, setMessage] = useState<string | null>(null);
  const debouncedSearch = useDebounce(search);
  const canAuthorizeChanges = user?.role === 'COORDINATOR';
  const studentBindings = useQuery({
  ...
  const professorBindings = useQuery({
    queryKey: ['coordination', 'professor-device-bindings', debouncedSearch],
    queryFn: () => coordinationApi.professorDeviceBindings({ q: debouncedSearch || undefined }),
  ...
  const authorizeProfessorChange = useMutation({
    mutationFn: coordinationApi.authorizeProfessorDeviceChange,
    onSuccess: async (_, externalId) => {
      setMessage(`Cambio autorizado para el profesor ${externalId}. Podrá vincular su nuevo celular al iniciar sesión.`);
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['coordination', 'professor-device-bindings'] }),
        queryClient.invalidateQueries({ queryKey: ['coordination', 'infrastructure-summary'] }),
      ]);
    },
  });
  ...
  const requestProfessorDeviceChange = (externalId: string, name: string) => {
    setMessage(null);
    const confirmed = window.confirm(
      `¿Autorizar el cambio de celular para ${name}? El vínculo actual quedará revocado y sólo el próximo login de ese profesor podrá registrar otro celular.`,
    );
    if (confirmed) authorizeProfessorChange.mutate(externalId);
  };

  const isLoading = activeTab === 'students' ? studentBindings.isLoading : professorBindings.isLoading;
  const isError = activeTab === 'students' ? studentBindings.isError : professorBindings.isError;
        ...
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
              Cada alumno y profesor conserva un único celular autorizado. Autoriza un cambio sólo después de validar la identidad.
            </p>
          </div>
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-slate-200 p-5 dark:border-[#1f2229] sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Busca y revisa el dispositivo antes de autorizar el reemplazo.</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="inline-flex rounded-lg border border-slate-200 bg-slate-50 p-1 text-sm font-semibold dark:border-[#252932] dark:bg-[#15181d]">
              <button type="button" className={tabClass(activeTab === 'students')} onClick={() => { setActiveTab('students'); setMessage(null); }}>
                Alumnos
              </button>
              <button type="button" className={tabClass(activeTab === 'professors')} onClick={() => { setActiveTab('professors'); setMessage(null); }}>
                Profesores
              </button>
            </div>
            <label className="relative block sm:w-80">
              <span className="sr-only">{activeTab === 'students' ? 'Buscar matrícula' : 'Buscar profesor'}</span>
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17} />
              <input
                className="field pl-9"
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                placeholder={activeTab === 'students' ? 'Buscar matrícula' : 'Buscar profesor'}
              />
            </label>
          </div>
        </div>
        ...
        {(authorizeChange.isError || authorizeProfessorChange.isError) && (
        ...
        {isLoading ? (
        ...
        ) : activeTab === 'students' && !studentBindings.data?.data.length ? (
          <div className="p-5">
        ...
        ) : activeTab === 'professors' && !professorBindings.data?.data.length ? (
          <div className="p-5">
            <EmptyState icon={<Link2 />} title="Sin profesores vinculados" description="Los profesores aparecerán después de iniciar sesión en la app de profesores." />
          </div>
        ) : activeTab === 'students' ? (
          <div className="divide-y divide-slate-100 dark:divide-[#1f2229]">
            {studentBindings.data!.data.map((binding) => (
          ...
          <div className="divide-y divide-slate-100 dark:divide-[#1f2229]">
            {professorBindings.data!.data.map((binding) => (
              <article key={binding.id} className="grid gap-4 p-5 lg:grid-cols-[1fr_1.2fr_auto] lg:items-center">
                <div className="min-w-0">
                  <p className="font-semibold">{binding.name}</p>
                  <p className="mt-1 truncate text-xs text-slate-500" title={binding.email ?? undefined}>{binding.email || 'Correo no registrado'}</p>
                  <p className="mt-1 font-mono text-xs text-slate-500">ID {binding.externalId}</p>
                </div>
                <div className="flex min-w-0 items-start gap-3">
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-slate-100 text-slate-500 dark:bg-[#22252b]"><Smartphone size={18} /></span>
                  <div className="min-w-0 text-sm">
                    <p className="font-medium">{formatPlatform(binding.platform)}</p>
                    <p className="truncate text-xs text-slate-500" title={binding.deviceInfo ?? undefined}>{binding.deviceInfo || 'Información del dispositivo no disponible'}</p>
                    <p className="mt-1 truncate font-mono text-xs text-slate-500" title={binding.deviceBindingId ?? undefined}>{binding.deviceBindingId || 'Sin celular vinculado'}</p>
                  </div>
                </div>
                <Button
                  variant="secondary"
                  disabled={!canAuthorizeChanges || authorizeProfessorChange.isPending || !binding.deviceBindingId}
                  onClick={() => requestProfessorDeviceChange(binding.externalId, binding.name)}
                  title={canAuthorizeChanges ? 'Revocar el celular actual y permitir un nuevo vínculo' : 'Tu cuenta es de sólo lectura'}
                >
                  Autorizar cambio
                </Button>
              </article>
...
function tabClass(active: boolean) {
  return [
    ...
    active
      ? 'bg-white text-[#C8102E] shadow-sm dark:bg-[#252932]'
