...
export async function exportReportExcel(report: WeeklyReportResponse): Promise<void> {
  ...
  for (const reportRow of report.data.rows) {
    const row = sheet.addRow([reportRow.startTime && reportRow.endTime ? `${reportRow.startTime}-${reportRow.endTime}` : reportRow.rawSchedule, reportRow.subject, reportRow.groupCode, reportRow.classroom || '—', ...days.map((day) => statusLabels[reportRow.cells[day.key]?.status ?? 'NOT_SCHEDULED'])]);
