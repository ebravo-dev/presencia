...
export async function exportReportExcel(report: WeeklyReportResponse): Promise<void> {
  ...
  sheet.eachRow((row, number) => { if (number > 4) row.eachCell((cell) => { cell.alignment = { vertical: 'middle', wrapText: true }; cell.border = { bottom: { style: 'thin', color: { argb: 'FFE2E8F0' } } }; }); });
...
export function exportReportPdf(report: WeeklyReportResponse): void {
  ...
  doc.setFontSize(8); doc.setTextColor(100); doc.text(`Generado ${new Date(report.meta.generatedAt).toLocaleString('es-MX')} · Zona horaria ${report.meta.timezone}`, 14, 200); doc.save(filename(report, 'pdf'));
}
