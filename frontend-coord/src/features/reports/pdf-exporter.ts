...
export async function exportReportPdf(report: WeeklyReportResponse): Promise<void> {
  ...
  autoTable(doc, {
    ...
    body: report.data.rows.map((row) => [
      ...
      ...days.map((day) => row.cells[day.key].status),
    ]),
...
function drawStatusMark(doc: jsPDF, data: CellHookData, report: WeeklyReportResponse): void {
  ...
  if (!day || !row) return;
  const status = row.cells[day.key].status;
