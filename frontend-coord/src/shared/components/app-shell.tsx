import { BarChart3, BookOpenCheck, LogOut, Menu, Settings2, X } from 'lucide-react';
...
const navigation = [
  ...
  { to: '/infraestructura', label: 'Infraestructura', icon: Settings2 },
  { to: '/reportes/asistencia', label: 'Reporte semanal', icon: BarChart3 },
];
const pageTitles: Record<string, string> = { '/': 'Resumen institucional', '/carga-academica': 'Carga académica', '/infraestructura': 'Infraestructura BLE', '/reportes/asistencia': 'Asistencia semanal' };
...
export function AppShell() {
  ...
  return <div className="min-h-screen bg-[#F8FAFC] text-[#111111]">
    ...
    <div className="lg:pl-72"><header className="sticky top-0 z-20 flex h-20 items-center gap-4 border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-8"><button className="rounded-lg p-2 hover:bg-slate-100 lg:hidden" onClick={() => setOpen(true)} aria-label="Abrir menú"><Menu /></button><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-[#C8102E]">Coordinación Académica</p><h1 className="text-xl font-bold sm:text-2xl">{pageTitles[location.pathname] ?? 'Panel administrativo'}</h1></div></header><main className="p-4 sm:p-8"><Outlet /></main></div>
  </div>;
}
