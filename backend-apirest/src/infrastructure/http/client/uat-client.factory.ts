...
export class UatClientFactory {
  ...
  private fromJar(jar: CookieJar): UatPortalClientPort {
    return new UatPortalClient({
      // Demo mode only changes how attendance uploads are persisted. Login and
      // every read-only academic request must still use the institutional UAT
      // portal so real credentials keep working in demonstrations.
