...
export async function seedSharedClassData(): Promise<void> {
  const prisma = new PrismaClient();

  try {
    const cycle = {
      externalId: readEnv('SEED_CYCLE_EXTERNAL_ID', '150'),
      name: readEnv('SEED_CYCLE_NAME', '2026 - 1 PRIMAVERA').toUpperCase(),
    };
    ...
    const group = await prisma.groupAssignment.upsert({
      ...
      create: {
        ...
        groupCode: 'Z',
        schoolCycleExternalId: cycle.externalId,
...
if (import.meta.url === entryPoint) {
  void seedSharedClassData().catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
}
